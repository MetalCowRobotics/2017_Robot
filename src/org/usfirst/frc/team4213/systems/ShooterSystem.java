...
public class ShooterSystem implements Subsystem{
	...
	public ShooterSystem(){
		...
		shooterTBH = new TBHController("shooter",tbhKi, true);
		...
		hoodPID = new PIDController("hood",hoodKp,hoodKi,hoodKd,hoodILife,true);
	}
	...
	void runHoodPIDLoop(){
		...
		SmartDashboard.putNumber("Output Hood Power",outputPWM);
...
//		Shooter.INSTANCE.setHoodSpeed(hoodSpeed);
		double hoodTarget = SmartDashboard.getNumber("hood.target",0);
		hoodPID.setTarget(hoodTarget);
		switch(state){ 
	    case IDLE:  
